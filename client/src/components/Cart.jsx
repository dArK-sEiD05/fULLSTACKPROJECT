...
const  handleCheckOut=()=>{
  ...
  return (
    <motion.div {...slideIn} className="fixed z-50 top-0 right-0 w-300 md:w-508 bg-lightOverlay backdrop-blur-md shadow-md h-screen">
        <div className="w-full flex items-center justify-between  pb-12 px-6">
        <motion.i
          {...buttonClick}
        ...
        <div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-6 gap-14">
        ...
        <h1 className='text-3xl flex justify-center items-center text-primary font-bold '>Empty Cart !!</h1>
        </>)}
        </div>
...
export const CartItemCard=({index,data})=>{
  ...
  return (
    <motion.div key={index} className='w-full flex items-center justify-start bg-zinc-800 rounded-md drop-shadow-md px-4 gap-4' {...staggerFadeInOut(index)}>
       <img
        src={data?.imageURL}
      ...
      <div className="flex items-center justify-start gap-1 w-full">
        <p className="text-lg text-primary font-semibold">
          {data?.product_name}{" - "}{data.product_price}{" - "}{data.quantity}
